...
class DeepPunctuation(nn.Module):
    def __init__(self, pretrained_model, freeze_bert=False):
        ...
        self.linear = nn.Linear(in_features=bert_dim * 2, out_features=len(punctuation_dict))
...
class DeepPunctuationCRF(nn.Module):
    ...
    def forward(self, x, attn_masks, y):
        ...
        x = torch.transpose(x, 0, 1)
        x = self.linear(x)
        # CRF requires byte tensor for mask
        attn_masks = attn_masks.byte()
        crf_out = self.crf.forward(x, y, mask=attn_masks, reduction='token_mean')
        y_pred = None
        # pad list of integers to have equal length and convert to torch tensor
