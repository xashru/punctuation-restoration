...
# This file is used to restore punctuations to sr test set using the bag of words approach
# it is the same approach as restore_punctuations_to_sr_train.py 

test_file = 'Switchboard_data_split_for_sentence_unit_detection\stm'
save_path = 'Switchboard_data_split_for_sentence_unit_detection/stm_restored_by_windowing_version2'
...
def get_all_lines_with(file_path, target_title):
	...
	relevant_lines = list(map(lambda x: x.split(" ", 5)[-1].lower().strip(), relevant_lines))
...
def get_restored_segments(sr_segments, sr_segments_with_punc):
	'''
	:sr_segments: sr segments separated by lines
...
test_set = get_full_set(test_file)																# read in full test set from sr data
for root, dirs, files in os.walk(utt_path):														# loop through every file in original utt data
	for filename in files:										
		...
		if formatted_filename[:-4] in test_set:													# Handle if file is in test set
			train_segment_1 = get_all_lines_with(test_file, formatted_filename[:-4] + '-A')		# get speaker A lines from sr data
			train_segment_2 = get_all_lines_with(test_file, formatted_filename[:-4] + '-B')		# get speaker B lines from sr data
			...
			if segment_1_a_count > segment_1_b_count:											# segment 1 corresponds to a_segments in original
			...
			else:																				# segment 1 corresponds to b_segments in original
			...
			restored_segments_1 = get_restored_segments(train_segment_1, segment_1_words)
			restored_segments_2 = get_restored_segments(train_segment_2, segment_2_words)
			
			# save data
			test_restored = open(save_path, "a", encoding="utf-8")
			for i in restored_segments_1:
				test_restored.write(i)
				test_restored.write('\n')
			for j in restored_segments_2:
				test_restored.write(j)
				test_restored.write('\n')